...
for coordinate = 1:number_of_vars
    ...
    function point = random_point_from_granules(granules, index)
        ...
        pos_inf_index = find(intervals(interval_index) == +Inf);
        neg_inf_index = find(intervals(interval_index) == -Inf);
        if ~isempty(pos_inf_index)
            intervals(interval_index, pos_inf_index) = +999999;
        end
        if ~isempty(neg_inf_index)
            intervals(interval_index, neg_inf_index) = -999999;
        end
